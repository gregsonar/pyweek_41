...
class Player:
    ...
    def handle_keydown(self, key: int) -> None:
        match key:
            case pygame.K_f:
                self.toggle_lantern()
            case pygame.K_e:
                self._interact_cooldown = 0.3   # handled in update
    ...
    def handle_mouse(self, button: int, pos: tuple[int, int]) -> None:
        pass   # reserved for aiming lantern direction
    ...
    def draw(self, screen: pygame.Surface) -> None:
        # Placeholder: magenta square until sprites arrive
        color = (200, 180, 80) if self.lantern_on else (150, 140, 100)
        pygame.draw.rect(screen, color, self.rect)
        # HP pips above player
        for i in range(self.hp):
            pygame.draw.circle(screen, (220, 60, 60), (self.rect.x + 6 + i * 10, self.rect.top - 8), 4)
